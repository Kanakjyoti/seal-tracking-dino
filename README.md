...
- DINOv2 attention maps localize seals with zero manual input that is no seed points, no annotations needed.
- Attention weakens proportionally when seals are partially submerged, accurately reflecting reduced visual distinctiveness.
- FeatUp produces genuinely sharper boundaries than simple input upscaling which is confirmed visually (PCA, similarity maps) and quantitatively (boundary sharpness plot).
